    ...
    class Initiator(val po: PurchaseOrderState,
        ...
        override fun call(): ExampleFlowResult {
            ...
            try {
                ...
                //ORACLE
                val transBuilder = TransactionType.General.Builder(notary)
                val oracle = serviceHub.networkMapCache.getNodesWithService(ServiceType.corda.getSubType("agb_register_oracle_service")).first()
                val oracleParty = oracle.serviceIdentities(ServiceType.corda.getSubType("agb_register_oracle_service")).first()

                fun filterCommands(c: Command) = oracleParty.owningKey in c.signers && c.value is AgbEntry

                val filterFuns = FilterFuns(filterCommands = ::filterCommands)
                //TODO temp agbobject to pass, has to come from form
                val formInputAGB = po.purchaseOrder.agbCode
                val agbObject = AgbEntryOf(agbCode = formInputAGB, geldigVan = LocalDate.of(2016, 5, 1), geldigTot = LocalDate.of(2018, 1, 1))

                val addOracle = object : OracleFlow(transBuilder, filterFuns, oracleParty, agbObject) { }
                subFlow(addOracle)
