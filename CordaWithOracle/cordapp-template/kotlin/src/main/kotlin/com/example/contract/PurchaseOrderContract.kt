        ...
        class Place : Clause<PurchaseOrderState, Commands, UniqueIdentifier>() {
            override fun verify(tx: TransactionForContract,
                ...
                requireThat {
                    ...
                    "Alleen NL is toegestaan." by (out.purchaseOrder.deliveryAddress.country == "NL")
                    "Minimaal 1 item." by (out.purchaseOrder.items.isNotEmpty())
                    "Minimaal aantal items = 1." by (out.purchaseOrder.items.map(Item::amount).all { it > 0 })
                    "Maximaal 100 items in totaal." by (out.purchaseOrder.items.map(Item::amount).sum() <= 100)
                    val time = tx.timestamp?.midpoint
                    "Ingangsdatum moet in de toekomst liggen." by (out.purchaseOrder.deliveryDate.toInstant() > time)
                }
