 ...
 */
open class DeclaratieContract() : Contract {
    /**
    ...
    interface Commands : CommandData {
        data class Indienen(override val nonce: Long = random63BitValue()) : IssueCommand, Commands
...
//        data class Amend(): TypeOnlyCommandData, Commands
    ...
    interface Clauses {
        ...
        class Group : GroupClauseVerifier<DeclaratieState, Commands, UniqueIdentifier>(AnyComposition(Indienen())) {
            override fun groupStates(tx: TransactionForContract): List<TransactionForContract.InOutGroup<DeclaratieState, UniqueIdentifier>>
                    // Group by purchase order linearId for in / out states
                    = tx.groupStates(DeclaratieState::linearId)
        }

        class Indienen : Clause<DeclaratieState, Commands, UniqueIdentifier>() {
            override fun verify(tx: TransactionForContract,
                                inputs: List<DeclaratieState>,
                                outputs: List<DeclaratieState>,
                                commands: List<AuthenticatedObject<Commands>>,
                                groupingKey: UniqueIdentifier?): Set<Commands> {
                val command = tx.commands.requireSingleCommand<Commands.Indienen>()
                requireThat {
