...
class DeclaratieApi(val services: ServiceHub) {
    ...
    fun createPurchaseOrder(po: Declaratie, @PathParam("party") partyName: String): Response {
        ...
        if(otherParty != null) {
            ...
            val result: DeclaratieFlowResult = services.invokeFlowAsync(DeclaratieFlow.Zorgaanbieder::class.java, state, otherParty).resultFuture.get()
            when (result) {
...
//        val zorgverzekeraarParty = services.identityService.partyFromName(partyName)
//        if(zorgverzekeraarParty != null) {
//            val state = PurchaseOrderState(po, services.myInfo.legalIdentity, zorgverzekeraarParty, PurchaseOrderContract())
//            // The line below blocks and waits for the future to resolve.
//            val result: ExampleFlowResult = services.invokeFlowAsync(ExampleFlow.Zorgaanbieder::class.java, state, zorgverzekeraarParty).resultFuture.get()
//            when (result) {
