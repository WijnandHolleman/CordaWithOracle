 ...
 *
 * It is a generic flow which facilitates the workflow required for two parties; an [Zorgaanbieder] and an [Zorgverzekeraar],
 * to come to an agreement about some arbitrary data (in this case, a [PurchaseOrder]) encapsulated within a [DealState].
 *
 * As this is just an example there's no way to handle any counter-proposals. The [Zorgverzekeraar] always accepts the
 ...
 */
object DeclaratieFlow {

    //data class SignRequest(val rootHash: SecureHash, val ftx: FilteredTransaction)

    class Zorgaanbieder(val po: DealState,
                        val zorgverzekeraarParty: Party,
                        override val progressTracker: ProgressTracker = Zorgaanbieder.tracker()): FlowLogic<DeclaratieFlowResult>() {
        /**
        ...
        companion object {
             ...
             *  INDIENEN_DECLARATIE
             *  AGB_CONTROLE
             *  VERSTUREN_DECLARATIE
             *
             */

            object INDIENEN_DECLARATIE : ProgressTracker.Step("Indienen declaratie")
            object AGB_CONTROLEREN : ProgressTracker.Step("AGB gegevens controleren")
            object VERSTUREN_DECLARATIE : ProgressTracker.Step("Versturen declaratie naar zorgverzekeraar")
...
//            )
         ...
         */
        @Suspendable
        override fun call(): DeclaratieFlowResult {
            ...
            try {
                ...
                // Stage 1.
                progressTracker.currentStep = INDIENEN_DECLARATIE
                ...
                // -----------------------
                // Flow jumps to Zorgverzekeraar.
                // -----------------------
                progressTracker.currentStep = RECEIVED_PARTIAL_TRANSACTION
                val ptx = sendAndReceive<SignedTransaction>(zorgverzekeraarParty, offerMessage).unwrap {
                //val ptx = sendAndReceive<SignedTransaction>(zorgverzekeraarParty, offerMessage).unwrap {
                    // Stage 7.
...
//                val oracleParty = oracle.serviceIdentities(handshake.payload.oracleType).first()
                ...
                send(zorgverzekeraarParty, ntx)
                return DeclaratieFlowResult.Success("Transaction id ${ntx.id} committed to ledger.")
            } catch(ex: Exception) {
                // Just catch all exception types.
                return DeclaratieFlowResult.Failure(ex.message)
            }
        }
    }

    class Zorgverzekeraar(val zorgaanbiederParty: Party,
                          override val progressTracker: ProgressTracker = Zorgverzekeraar.tracker()): FlowLogic<DeclaratieFlowResult>() {
        companion object {
        ...
        override fun call(): DeclaratieFlowResult {
            try {
                ...
                val message = receive<TransactionState<DealState>>(zorgaanbiederParty).unwrap { it }
 ...
 */
sealed class DeclaratieFlowResult {
    class Success(val message: String?): DeclaratieFlowResult() {
        override fun toString(): String = "Success($message)"
    }

    class Failure(val message: String?): DeclaratieFlowResult() {
