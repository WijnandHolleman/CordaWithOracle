...
import net.corda.core.node.services.ServiceType.Companion.corda
import net.corda.core.transactions.FilterFuns
import net.corda.core.transactions.FilteredTransaction
import net.corda.core.utilities.DUMMY_NOTARY
import net.corda.core.utilities.LogHelper
import net.corda.irs.api.NodeInterestRates
import net.corda.irs.api.VektisAGBValues
import net.corda.irs.flows.RatesFixFlow
import net.corda.irs.flows.VektisFixFlow
...
class NodeInterestRatesTest {
    ...
    val TEST_DATA_VEKTIS = VektisAGBValues.parseFile("""
        Garant 2016-03-16 = 122344
        Thebe 2016-03-16 = 849339
        Orthi 2017-05-30 = 467223
        """.trimIndent())
...
//        val fixOf = NodeInterestRates.parseFixOf("LIBOR 2016-03-16 5Y")
...
//    }
    ...
    fun `network CZ`() {
        val net = MockNetwork()
        val n1 = net.createNotaryNode()
        val n2 = net.createNode(n1.info.address, advertisedServices = ServiceInfo(VektisAGBValues.type))
       databaseTransaction(n2.database) {
            n2.findService<VektisAGBValues.Service>().oracle.knownFixes = TEST_DATA_VEKTIS
        }
        val tx = TransactionType.General.Builder(null)
        val fixOf = VektisFixOf("Garant", LocalDate.parse("2016-03-16"))
        val oracle = n2.info.serviceIdentities(VektisAGBValues.type).first()
        fun filterCommands(c: Command) = oracle.owningKey in c.signers && c.value is Fix
        val filterFuns = FilterFuns(filterCommands = ::filterCommands)
        val flow = VektisFixFlow(tx, filterFuns, oracle, fixOf, "0.675".bd, "0.1".bd)
        LogHelper.setLevel("rates")
        net.runNetwork()
        val future = n1.services.startFlow(flow).resultFuture
        net.runNetwork()
