    ...
    class Service(val services: PluginServiceHub) : SingletonSerializeAsToken() {
        ...
        private class FixQueryHandler(val otherParty: Party, val service: Service) : FlowLogic<Unit>() {
            ...
            override fun call(): Unit {
                val request = receive<RatesFixFlow.VektisQueryRequest>(otherParty).unwrap { it }
...
//        }
    ...
    class Oracle(val identity: Party, private val signingKey: KeyPair, val clock: Clock) {
        private object Table : JDBCHashedTable("demo_vektis_agb_fixes") {
            val name = varchar("index_name", length = 255)
            val forDay = localDate("for_day")
            //val ofTenor = varchar("of_tenor", length = 16)
            val value = varchar("value", length = 10)
        }

        private class InnerState {
            val fixes = object : AbstractJDBCHashSet<VektisFix, Table>(Table) {
                override fun elementFromRow(row: ResultRow): VektisFix {
                    return VektisFix(VektisFixOf(row[table.name], row[table.forDay]), row[table.value])
                }

                override fun addElementToInsert(insert: InsertStatement, entry: VektisFix, finalizables: MutableList<() -> Unit>) {
                    insert[table.name] = entry.of.name
                    insert[table.forDay] = entry.of.forDay
                    insert[table.value] = entry.value
                }
            }
            var container: VektisFixContainer = VektisFixContainer(fixes)
        }

        private val mutex = FiberBox(InnerState())

        var knownFixes: VektisFixContainer
            set(value) {
         ...
         */
        @Suspendable
        fun query(queries: List<VektisFix>, deadline: Instant): List<VektisFix> {
            require(queries.isNotEmpty())
            return mutex.readWithDeadline(clock, deadline) {
                val answers: List<VektisFix?> = queries.map { container[it.of] }
        ...
        fun sign(ftx: FilteredTransaction, merkleRoot: SecureHash): DigitalSignature.LegallyIdentifiable {
            ...
            val fixes: List<VektisFix> = ftx.filteredLeaves.commands.
                    filter { identity.owningKey in it.signers && it.value is Fix }.
                    map { it.value as VektisFix }
            ...
            for (fix in fixes) {
                val known = knownFixes.get(fix.of)
                if (known == null || known != fix)
                    throw UnknownFix(fix)
            }
    ...
    class UnknownFix(val fix: VektisFix) : RetryableException("Unknown fix: $fix")

    /** Fix container, for every fix name & date pair stores a tenor to interest rate map - [InterpolatingRateMap] */
    class FixContainer(val fixes: Set<VektisFix>, val factory: InterpolatorFactory = CubicSplineInterpolator) {
        private val container = buildContainer(fixes)
        val size: Int get() = fixes.size

        operator fun get(fixOf: VektisFixOf): VektisFix? {
            for (fix in container) {
                if(fix.of == fixOf)
                {
                    return fix;
                }
        ...
        private fun buildContainer(fixes: Set<VektisFix>): List<VektisFix> {
            var FixList :MutableList<VektisFix> = mutableListOf()
    ...
    class VektisFixContainer(val fixes: Set<VektisFix>, val factory: InterpolatorFactory = CubicSplineInterpolator) {
        private val container = buildContainer(fixes)
        val size: Int get() = fixes.size

        operator fun get(fixOf: VektisFixOf): VektisFix? {
            val rates = container[fixOf.name to fixOf.forDay]
            val fixValue = rates?.getRate(fixOf.forDay) ?: return null
            return VektisFix(fixOf, fixValue)
        }

        private fun buildContainer(fixes: Set<VektisFix>): Map<Pair<String, LocalDate>, InterpolatingRateMap> {
            val tempContainer = HashMap<Pair<String, LocalDate>, HashMap<LocalDate, String>>()
            for (fix in fixes) {
                val fixOf = fix.of
                val rates = tempContainer.getOrPut(fixOf.name to fixOf.forDay) { HashMap<LocalDate, String>() }
                rates[fixOf.forDay] = fix.value
            }
    ...
    class InterpolatingRateMap(val date: LocalDate,
         ...
         */
        fun getRate(tenor: LocalDate): String? {
        ...
        private fun daysToMaturity(tenor: LocalDate) = daysToMaturity(date, calendar)

        private fun interpolate(tenor: LocalDate): String? {
    ...
    fun parseFile(s: String): VektisFixContainer {
        val fixes = s.lines().
                map(String::trim).
                ...
                map { parseFix(it) }.
                toSet()
        return VektisFixContainer(fixes)
    }

    /** Parses a string of the form "LIBOR 16-March-2016 1M = 0.678" into a [Fix] */
    fun parseFix(s: String): VektisFix {
        val (key, value) = s.split('=').map(String::trim)
        val of = parseFixOf(key)
        val code = value
        return VektisFix(of, code)
    }

    /** Parses a string of the form "LIBOR 16-March-2016 1M" into a [FixOf] */
    fun parseFixOf(key: String): VektisFixOf {
        val words = key.split(' ')
        val date = words.last()
        val name = words.dropLast(1).last()
        val d = LocalDate.parse(date)
        return VektisFixOf(name, LocalDate.parse(date))
    }
}
