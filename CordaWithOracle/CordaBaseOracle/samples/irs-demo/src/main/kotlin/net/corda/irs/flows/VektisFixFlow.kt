 ...
 */
open class VektisFixFlow(protected val tx: TransactionBuilder,
    ...
    data class QueryRequest(val queries: List<VektisFixOf>, val deadline: Instant)
    data class VektisQueryRequest(val queries: List<VektisFixOf>, val deadline: Instant)
     ...
     */
    @Suspendable
    protected open fun beforeSigning(fix: VektisFix) {
    }

    private fun checkFixIsNearExpected(fix: VektisFix) {
        if (false) {
            // TODO: Kick to a user confirmation / ui flow if it's out of bounds instead of raising an exception.
            throw FixOutOfRange(BigDecimal(999))
        }
    }

    // DOCSTART 1
    class FixQueryFlow(val fixOf: VektisFixOf, val oracle: Party) : FlowLogic<VektisFix>() {
        @Suspendable
        override fun call(): VektisFix {
            val deadline = suggestInterestRateAnnouncementTimeWindow(fixOf.name, oracle.name, fixOf.forDay).end
            // TODO: add deadline to receive
            val resp = sendAndReceive<ArrayList<VektisFix>>(oracle, VektisQueryRequest(listOf(fixOf), deadline))
